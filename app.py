...
#     refine_messages.append({
#         "role": "user",
#         "content": f"Refine this prompt based on the previous conversation, it should gave URL of souurce: {input_text}",
#     })
...
#     search_content = "\n".join([result.get('title', '') + "\n" + result.get('snippet', '') for result in search_results["text_results"]])
...
#     refine_messages.append({
#         "role": "user",
#         "content": f"Refine this prompt based on the previous conversation, it should also gave URL of souurce: {input_text}",
#     })
...
#     refine_messages.append({
#         "role": "user",
#         "content": f"Refine this prompt based on the previous conversation and also share source links: {input_text}",
#     })
...
# def is_significantly_different(new_prompt, chat_history, threshold=0.5):
#     """Determine if the new prompt is significantly different from the previous history."""
#     if not chat_history:
#         return True
#     last_prompt = chat_history[-1]['prompt']
#     similarity = difflib.SequenceMatcher(None, new_prompt, last_prompt).ratio()
...
#     if is_significantly_different(input_text, chat_history):
#         refine_messages = [{"role": "user", "content": input_text}]
#     else:
#         # Prepare messages for Groq API to refine the search query
#         refine_messages = []
#         # Include previous chat history in the messages
